import convertUnixTimeToDateTime from "./convertUnixTimeToDateTime";
export function deserializeWeatherDayInfo(weatherData) {
  ...
  return {
    name,
    coord,
    dt: convertUnixTimeToDateTime(dt).formattedDateTime,
    condition,
    description,
...
export function deserializeWeatherForecastInfo(weatherForecastData) {
  ...
  const todayDay = convertUnixTimeToDateTime(
  ...
  for (let i = 0; i < list.length; i++) {
    let convertedForecastDt = convertUnixTimeToDateTime(
      list[i].dt
    ).formattedDay;
